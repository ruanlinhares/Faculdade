/****************************************************************** 
 * Nome: Lista simplesmente encadeada/circular com apontadores    *
...
int main ()
{
	...
	do
	{
		...
		case 3 :
			...
			scanf("%f", &novaMoto.preco);
			printf("\n\n");
			printf("Informe o codigo da moto que deseja inserir a nova moto em sua posicao: ");
			scanf("%d", &cod_busca);
...
int inserirElemento(Lista *lista, Motocicleta novaMoto)
{
	...
	if(lista->prim == NULL)
	{
		...
		//faz o unico elemento da lista tambem ser o ultimo
		lista->ult = novoEspaco;
		//faz o ultimo elemento da lista apontar para o primeiro
	...
	p->prox = novoEspaco;
	//coloca o novo elemnto adicionado como ultimo elemento da lista;
	lista->ult = p->prox; 
	//faz o prox do novo elemento apontar para o primeiro elemento;
	lista->ult->prox = lista->prim;
	return 1;
}
...
int inserirElementoPosi(Lista *lista, Motocicleta novaMoto, int cod_busca)
{
	...
	if(lista->prim->novaMoto.cod_moto == cod_busca){
		...
		lista->prim = novoEspaco;
		//faz o ultimo elemento apontar para o primeiro
		lista->ult->prox = lista->prim;
		
		return 1;
	}

	//Loop percorre a lista
	
	p = lista->prim;

	do{	
		//verifica se p é o ultimo elemento
		if(cod_busca == p->novaMoto.cod_moto){
			
			if(p->prox == lista->prim){
				//faz o elemento antual apontar para o novo elemento	
				p->prox = novoEspaco;
				//faz lista ult receber o ultimo elemento adicionado
				lista->ult = novoEspaco;
				//faz o ultimo elemento apontar para o primeiro
				lista->ult->prox = lista->prim;
			}else{
			
				//se sim faz o novo elemento apontar para o proximo no do elemento atual;
				novoEspaco->prox = p->prox;
				//Faz o elemento atual apontar para o novo elemento;
				p->prox = novoEspaco;
...
int inserirElementoInicio(Lista * lista, Motocicleta novaMoto)
{
	...
	//faz novo elemento apontar para o primeiro elemento;
	novoEspaco->prox = lista->prim;
...
int removerElemento(Lista *lista, int cod_busca)
{
	...
	do{
		...
		// Verifica se o próximo elemento é o desejado
        if (p->prox->novaMoto.cod_moto == cod_busca) {
        	
			ref = p->prox; // Guarda o elemento a ser removido
  
         	// Ajusta o ponteiro para "pular" o elemento removido
        	p->prox = p->prox->prox;
  
            free(ref); // Libera o elemento removido
            return 1;
        }
